...
const dispatch=useDispatch();
 ...
 const totalPrice=cartItems.reduce((acc,num)=>acc+num.newPrice,0)
...
const {
    ...
    const onSubmit = async(data) => {
        ...
        const newOrder = {
            ...
            phone: data.phone,
            productIds: cartItems.map(item => item?._id), title: { type: String, required: true },
            totalPrice: totalPrice
        }
        console.log(newOrder);
        try{
          await createOrder(newOrder).unwrap();
          Swal.fire("Order is placed");
          navigate('/orders');
